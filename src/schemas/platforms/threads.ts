...
// Standard post URL: threads.net/@{username}/post/{postId}
const threadsPostUrlSchema = z
	.string()
	.regex(/threads\.net\/@[^/]+\/post\/[A-Za-z0-9_-]+/i, {
		message: 'Invalid Threads post URL format (expected: /@{username}/post/{postId})',
	});

// Thread URL using /t/ path: threads.net/t/{threadId}
const threadsThreadUrlSchema = z
	.string()
	.regex(/threads\.net\/t\/[A-Za-z0-9_-]+/i, {
		message: 'Invalid Threads thread URL format (expected: /t/{threadId})',
	});

// Direct post link format: threads.net/{postId}
const threadsDirectUrlSchema = z
	.string()
	.regex(/threads\.net\/[A-Za-z0-9_-]+$/i, {
