...
export class RetryService implements IService {
  ...
  async cleanup(): Promise<void> {
    ...
    console.log('[RetryService] Cleaned up');
  }
  ...
  async executeWithRateLimit<T>(
    ...
    const rateLimitConfig: RetryConfig = {
      ...
      onRetry: (attempt: number, error: Error, delay: number) => {
        // Handle rate limit specific retry notification
        if (error instanceof RateLimitError) {
          console.log(
            `[RetryService] Rate limited, waiting ${delay}ms before retry ${attempt}`
          );
        }
  ...
  private checkCircuit(key: string): void {
    ...
    if (circuit.state === CircuitState.OPEN) {
      ...
      if (
        ...
        circuit.successes = 0;
        console.log(`[RetryService] Circuit ${key} transitioning to HALF_OPEN`);
      } else {
  ...
  private recordSuccess(key: string): void {
    ...
    if (
      ...
      circuit.successes = 0;
      console.log(`[RetryService] Circuit ${key} CLOSED after successful recovery`);
    }
  }
  ...
  private recordFailure(key: string): void {
    ...
    if (circuit.state === CircuitState.HALF_OPEN) {
      ...
      circuit.successes = 0;
      console.log(`[RetryService] Circuit ${key} reopened after failure in HALF_OPEN state`);
      return;
    }
    ...
    if (
      ...
      circuit.state = CircuitState.OPEN;
      console.log(
        `[RetryService] Circuit ${key} OPENED after ${circuit.failures} consecutive failures`
      );
    }
  }
  ...
  resetCircuit(key: string): void {
    ...
    console.log(`[RetryService] Circuit ${key} manually reset to CLOSED`);
  }
