...
export class ErrorTracker implements IService {
  ...
  setDebugMode(enabled: boolean): void {
    ...
    console.log(`[ErrorTracker] Debug mode ${enabled ? 'enabled' : 'disabled'}`);
  }
  ...
  clearLogs(): void {
    this.errorLog = [];
    this.errorCounts.clear();
    console.log('[ErrorTracker] Error logs cleared');
  }
  ...
  private logToConsole(entry: ErrorLogEntry): void {
    ...
    console.group(prefix, entry.error.message);
    console.error('Error:', entry.error);
    console.log('Context:', entry.context);
    console.log('User Action:', entry.userAction);
